					...
					<c:forEach var="farVO" items="${list}" varStatus="vs">
						...
						<c:if test="${faSvc.getOneFa(farVO.faId).faStatus == 0}">
							<tr>
								...
								<td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal_1${vs.index}">文章詳情</button></td>
								<td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal${vs.index}">檢舉原因</button></td>


								<td><fmt:formatDate value="${farVO.farDate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<form method="post" action="<%=request.getContextPath()%>/back-end/forum/far.do">
									<td><select name="farStatus">
											<option value="0"></option>
											<option value="1">通過</option>
											<option value="2">不通過</option>
									</select></td>
									<td><input type="submit"> <input type="hidden" name="action" value="judge"> <input type="hidden" name="farId"
											value="${farVO.farId}"> <input type="hidden" name="faId" value="${farVO.faId}"> <input type="hidden" name="adminId"
											value="${adminsVO.admin_id}"></td> ${adminsVO.admin_id}
								</form>
							</tr>

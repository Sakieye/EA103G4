package com.usedbooktransaction.controller;
...
import com.usedbooktransaction.model.UsedbookTransactionService;
import com.usedbooktransaction.model.UsedbookTransactionVO;

@MultipartConfig(fileSizeThreshold = 1024 * 1024, maxFileSize = 5 * 1024 * 1024, maxRequestSize = 5 * 5 * 1024 * 1024)
public class UsedbookTransactionServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("delete".equals(action)) { // 來自listAllUsedbookTransaction.jsp
			...
			try {
				...
				/*************************** 2.開始刪除資料 ***************************************/
				UsedbookTransactionService usedbooktransactionSvc = new UsedbookTransactionService();
				usedbooktransactionSvc.deleteUsedbookTransaction(po_no);

				/*************************** 3.刪除完成，準備轉交(Send the Success view) ***********/
				String url = "/back-end/usedbooktransaction/listAllUsedbookTransaction.jsp";
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/usedbooktransaction/select_page_usedbooktransaction.jsp");
					failureView.forward(req, res);
					return;// 程式中斷
				}

...
public class AdDAO implements AdDAO_Interface {
	...
	private static final String INS_AD=
			"INSERT INTO ADVERTISEMENTS VALUES(('AD'||LPAD(ADID_SEQ.NEXTVAL,10,'0')),?,?,?,?,?,?,?,?,DEFAULT)";
	private static final String UPD_AD=
			"UPDATE ADVERTISEMENTS SET ACT_NAME=?, AD_TYPE=?, AD_START=?, AD_END=?, AD_IMAGE=?, AD_URL=?, AD_COPY=?, AD_UPDATE=DEFAULT WHERE AD_ID=?";
	private static final String DEL_AD=
			"DELETE FROM ADVERTISEMENTS WHERE AD_ID=?";
	private static final String FIND_TIME=
			"SELECT * FROM ADVERTISEMENTS WHERE AD_START BETWEEN ? AND ?";
	private static final String GET_ALL=
			"SELECT * FROM ADVERTISEMENYS ORDER BY AD_ID";
	...
	public void doCreate(AdVO advo) {
		...
		try {
			...
			pstmt.setByte(6, advo.getAd_image());
	...
	public void update(AdVO advo) {
		...
		try {
			...
			pstmt.setByte(5, advo.getAd_image());
	...
	public AdVO findbytime(Timestamp ad_start) {
		...
		try {
			...
			while(rs.next()) {
				...
				adVO.setAd_image(rs.getByte("ad_image"));
